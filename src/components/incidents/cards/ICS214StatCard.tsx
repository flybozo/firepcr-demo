      ...
      {filteredIcs.length === 0 ? (
        ...
        <>
          ...
          {filteredIcs.map(row => (
            ...
            >
              <span className="w-16 shrink-0 text-gray-400 text-xs">{fmtDateCompact(row.op_date)}</span>
              <span className="flex-1 min-w-0 text-gray-300 text-xs truncate">{row.unit_name}</span>
