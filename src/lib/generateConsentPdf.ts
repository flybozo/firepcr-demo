...
export function generateConsentToTreatPDF(data: ConsentToTreatData, logoDataUrl?: string | null): jsPDF {
  ...
  doc.text(`${data.agency_name || brand.companyLegal}  |  Medical Director: ${data.provider_name}`, W / 2, 38, { align: 'center' })
  ...
  const paragraphs = [
    `I, ${data.patient_name}, hereby consent to emergency medical treatment and care provided by ${brand.consentEntity} (${brand.companyLegal}) personnel, including physicians, physician assistants, nurse practitioners, registered nurses, EMTs, and paramedics.`,
    '',
  ...
  doc.text(`${brand.companyName} — Consent to Treat Form`, W / 2, 778, { align: 'center' })
